...
verify_and_append_content <-
  function(existing_content,
    ...
    if (verify_input(content_to_verify, verify_warning)) {
      ...
      return_content <-
        glue::glue('{existing_content}{is_invalid_insert}')
    }
...
create_html_list <- function(string_to_append, list_values, warning_text) {
  string_to_append <- glue::glue('{string_to_append}
                                 <ul>', .null = "NA")
  for (list_element in list_values) {
    if (verify_input(list_element, warning_text)) {
      string_to_append <- glue::glue('{string_to_append}
