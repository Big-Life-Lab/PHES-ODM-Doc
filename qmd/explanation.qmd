...
- **[Measures](https://docs.phes-odm.org/parts.html#measurements)**: A measurement or observation of any substance including a biological, physical or chemical substance.
- **[Methods](https://docs.phes-odm.org/parts.html#methods)**: A procedure for collecting a sample or performing a measure. 
- **[Attributes](https://docs.phes-odm.org/parts.html#attributes)**: A description of the who, where, when, and why of environmental surveillance. 

See the [parts reference document](ref-guide.qmd) for more details.

There are additional part types to support the dictionary including:

-   [Aggregations](https://docs.phes-odm.org/parts.html#aggregations)
-   [Aggregation scales](https://docs.phes-odm.org/parts.html#aggregationScales)
-   [Aggregation sets](https://docs.phes-odm.org/parts.html#aggregationSets)
-   [Categories](https://docs.phes-odm.org/parts.html#categories)
-   [Classes](https://docs.phes-odm.org/parts.html#classes)
-   [Compartments](https://docs.phes-odm.org/parts.html#compartments)
-   [Compartment sets](https://docs.phes-odm.org/parts.html#compartmentSets)
-   [Data types](https://docs.phes-odm.org/parts.html#dataTypes)
-   [Additional dictionary sheets](https://docs.phes-odm.org/parts.html#dictionarySupport)
-   [Dictionary set type](https://docs.phes-odm.org/parts.html#dictSets)
-   [Domains](https://docs.phes-odm.org/parts.html#domains)
-   [Exceptions Part Type](https://docs.phes-odm.org/parts.html#exceptions)
-   [Groups](https://docs.phes-odm.org/parts.html#groups)
-   [Missingness](https://docs.phes-odm.org/parts.html#missingness)
-   [Missingness sets](https://docs.phes-odm.org/parts.html#missingnessSets)
-   [Measure, method, or attribute sets](https://docs.phes-odm.org/parts.html#mmaSets)
-   [Nomenclatures](https://docs.phes-odm.org/parts.html#nomenclatures)
-   [Part Support](https://docs.phes-odm.org/parts.html#partSupport)
-   [Part types](https://docs.phes-odm.org/parts.html#partType)
-   [Quality indicators](https://docs.phes-odm.org/parts.html#qualityIndicators)
-   [Quality indicator sets](https://docs.phes-odm.org/parts.html#qualityIndSets)
-   [Short names](https://docs.phes-odm.org/parts.html#shortName)
-   [Shorthand or Short Name Set Type](https://docs.phes-odm.org/parts.html#shortSets)
-   [Specimens](https://docs.phes-odm.org/parts.html#specimens)
-   [Specimen sets](https://docs.phes-odm.org/parts.html#specimenSets)
-   [Tables](https://docs.phes-odm.org/parts.html#tables)
-   [Table Support](https://docs.phes-odm.org/parts.html#tableSupport)
-   [Units](https://docs.phes-odm.org/parts.html#units)
-   [Unit sets](https://docs.phes-odm.org/parts.html#unitSets)
...
-   [Aggregation sets](https://docs.phes-odm.org/parts.html#aggregationSets)
-   [Compartment sets](https://docs.phes-odm.org/parts.html#compartmentSets)
-   [Dictionary sets](https://docs.phes-odm.org/parts.html#dictSets)
-   [Missingness sets](https://docs.phes-odm.org/parts.html#missingnessSets)
-   [Measure, method, or attribute sets](https://docs.phes-odm.org/parts.html#mmaSets)
-   [Quality indicator sets](https://docs.phes-odm.org/parts.html#qualityIndSets)
-   [Shorthand or Short Name Sets](https://docs.phes-odm.org/parts.html#shortSets)
-   [Specimen sets](https://docs.phes-odm.org/parts.html#specimenSets)

Details about these sets are their similar - but unique - applications can be found in the at their individual links. Importantly, "Measure, method, or attribute sets", or "MMA sets" are a little different. See the section below for an explanation of this difference.

### Why MMA sets are a little different

One of the reasons why "sets" are set up and organized the way that they are, is to allow for the re-use of parts in multiple sets. "Measure, method, or attribute sets", or "MMA sets" are, however, an exception to this rule because the categories that make up a category set are - in general - used in only one set. Furthermore, the general sets are needed for almost every measure or method (which require unit, aggregation, quality, and compartment information), while MMA sets are used only in specific circumstances for specific fields. These are things such as collection metadata (example: Sample collection category set), dictionary metadata and model architecture (example: Data Type Category Set), or more detailed types of certain methods (example: Nucleic Acid Extraction Category Set). MMA sets are also a distinct part type, and more information about them can be found in the entry for [Measure, method, or attribute sets](https://docs.phes-odm.org/parts.html#mmaSets).

## The "what" and "why" of Specimen IDs

Specimen IDs indicate the level at which a measure is being done. This can be: a site measure, ie. a measure at a site such as temperature or weather; a sample measure, ie. a laboratory measurement on a sample, such as gene copies of SARS-CoV-2 per mL; a polygon measure, ie. case numbers within a health region's polygon; or a population measure, ie. an aggregate measure of a population, such as the number of confirmed cases of a given illness. The reason specimen IDs were created was so that there could be a single manner in which measures and methods are recorded, regardless of the level at which they were performed, while still maintaining the possibility to collect site, sample, and population measures as a distinct types of metadata.
...
Within the ODM there are three types of tables: program description tables, results tables, and look-up tables. These table types exist to differentiate the function of these tables and to highlight these differences to users. The different tables take different types of inputs and maintenance from users, [so understanding the differences can be important](https://docs.phes-odm.org/tables.html#table-types). 
...
Measures are actually types of measures that can be performed. These can range from temperature to the number of gene copies in a sample. The measure, or `measurement`, can be selected from a drop down in the templates and it specifies the kind of measure you intend to record. The actual value of the measure is then recorded in the `value` field of the `measures` table, with units and aggregation specified in the `unit` and `aggregation` fields.

### Methods

Similar to measures, methods are types of methods that can be performed to accomplish a measurement. These can be diverse, ranging from incubation, qPCR, or nucleic acid extraction. The method itself (`method`) is selected from a drop down in the templates, specifying in general terms the kind of method the user wishes to record. From there, the `value` field of the `protocolSteps` table can be populated by one of the inputs from the category set associated with that `method`. This provides a higher level of detail for a given method. For example, the `method` might be `solidSep` for solid separation of a sample. The value field might then be populated with `cent` for centrifugation, indicating more details about how the sample settling was done.
...
Within the ODM there is a `qualityFlag` field located in the `qualityReports` table. Each row of this table has a unique ID (`qualityReportID`) which can then be linked to specific measures, measure sets, and/or samples in the `measures`, `measureSets`, or `samples` tables. This field serves to highlight whether or not there is a quality issues with the sample or the measure(s). The quality flag also allows a user to specify the type of quality issue. This is managed through the use of quality sets, which are the sets that contain the possible quality flags for a given measure or for samples. The idea is that this provides data on any issues with a sample and an indication of the nature of that issue. Having data about the nature and presence of a quality issue is often not sufficient for decision makers who are trying to use and interpret the data. As such, there is also a `reportable` field which is a Boolean indicator of whether or not data can or should be reported or included in final reports and decisions, and a `severity` indicator with a "traffic light" style reporting to indicate how serious the issue is. In previous versions of the ODM, `qualityFlag` was included as an attirubute in the `measures` and `samples` tables. Separating it out like this allows for multiple flags to be noted on the same measure/sample, and a higher level of detail on the quality issues identified. It can be added back to be reported with measures, however, using a [wide table template](https://docs.phes-odm.org/wide-names.html). 
...
| Pipeline ID             | First Treatment ID in the pipeline + # treatments in the pipeline      | standardizationpmmov15            |
...
| Public Health Action ID | actionType + action + threatTarget + actionDT                          | survAlerttargSurvsarsCov228022027 |
| Quality Report ID       | quality flag part ID + either the measure report ID, the sample ID, or measure set report ID that the quality report is for                                                                        | flagJcaOnOttOttawasite1202008021000covN1002               |
...
| Treatment ID            | calcType + standard + order                                            | standardizationpmmov1             |

To go into greater detail, the *Accessions Index ID* is a unique ID for each row in the `accessions` table, and is made by concatenating the value of the `dataHost` field and the `accessNum` field. For example, an SRA entry with the accession index number of PRJNA788395 would give an accessions index ID of sraPRJNA788395.
...
For the *Calculation ID*, as a composite key, will just be generated by concatenating the pipelineID and the treatmentID of a given row. For example, if a row contains the pipeline ID "standardizationpmmov15" and the treatment ID "standardizationpmmov1", the calculation ID would be standardizationpmmov15standardizationpmmov1.
...
The *Pipeline ID* recommended recipe is to use the treatment ID of the first data treatment in the pipeline, and concatenate that with the number of treatments in the pipeline as a whole. For example, if the first treatment in a pipeline has the ID "standardizationpmmov1" and there are five treatments in the pipeline, the pipeline ID would be standardizationpmmov15.
...
For the *Public Health Action ID*, we recommend concatenating the value for the actionType field, the action field, the threatTarget, and the actionDT field (just the numbers). So if, for example, a surveillance alert (survAlert) action was taken to have targeted surveillance (targSurv) roll out for SARS-CoV-2 (sarsCov2) on February 28, 2027, the public health action ID would be survAlerttargSurvsarsCov228022027.

*Quality Report ID* ia generated using the recipe of part ID for the given quality flag + either the measure report ID, the sample ID, or measure set report ID for which the quality report is being generated. Generating an example, if we are reporting the qulaity flag J - Weak signal extrapolation (partID = flagJ) for a measure report with the ID caOnOttOttawasite1202008021000covN1002, we get the quality ID of flagJcaOnOttOttawasite1202008021000covN1002.
...
A *Site ID* can be generated by combining the country code (ISO) for the site + the province code (ISO) for the site + first 2-3 letters of municipality + 3 first letters of site name. So to generate a site ID for the wastewater treatment plant in Ottawa, Canada, we would take the ISO code for Canada (ca), for Ontario (on), The first three letters of Ottawa-Carleton (Ott) and the first three letters of the site name, where the name is Robert O. Pickard Environmental Centre (Rob). This gives us the recommended site ID of caOnOttRob. 

Lastly, a *Treatment ID* can be generated using the recipe of concatenating the value from the calcType field, the standard field, and then the value for the order in a pipeline. For example, if a data treatment that standardizes to circulating levels of pepper mild mottle virus (PMMoV) and is the first step in a data pipeline, its treatment ID would be standardizationpmmov1.
