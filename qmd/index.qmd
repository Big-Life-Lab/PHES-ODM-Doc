...
There are six document sections, each with a different purpose.[^index-1]
...
[*Background*](background.qmd) provides some additional context and explanation about the history and guiding principles of the PHES-ODM project. This section is not as important for users trying to get started with the model.
...
[*Sample relationships*](sampleRelationships.qmd) provides a deeper overview of sample relationships and sample provenance, and how to record it accurately in the ODM.

[*Long-format, wide-format, and wide-names*](wide-names.qmd) is an overview of the differences between "long" and "wide" formatted data, and how the ODM can be used in both formats. It also provides instructions on how to alternate reproducibly between the two.

[*Reference guide*](ref-guide.qmd) is a description of each part of the ODM. It serves as a reference for the other sections, and the definitive record of each part, table and set -- along with their attributes.

### Future documentation

Video tutorials and explanations are available in our [vimeo library](https://vimeo.com/user/126292027/folder/6496228?isPrivate=false), with new videos and tutorials still in development. Tutorials will help newcomers get started. Further documentation and lessons that are still forthcoming include items on how to:
...
-   Use of the `accessions`, and `calculations` tables.
-   Further guidance and walk-throughs on how to use ODM tools
...
![**Figure 1 - Key ODM tables for recording data**](assets/intro/figure1-ODMSubway-simple.png){fig-align="center"}
...
If you want to go deeper, the `Sites report table`([Sites](https://docs.phes-odm.org/tables.html#sites "Sites report table")) will capture data about where samples are collected, and the `Polygons report table`([Polygons](https://docs.phes-odm.org/tables.html#polygons "Polygons report table")) will record information about the regions that are represented by environmental samples.
...
In the ODM, tables are an example of a part. A part name can be identified by its text style like `this`. The part identifier often follows the part in parenthesis, with a link to the part in the reference section.
...
Keep in mind, ODM templates are only examples of how to use the ODM dictionary to record data. Templates are available at [the PHES-ODM OSF.io repository](https://osf.io/ab9se/files/osfstorage). The '[How to use the ODM](https://docs.phes-odm.org/how-to.html)" section describes how to enter information into tables.
...
A `measure` ([measure](parts.qmd#measureID)) is an observation or measurement of any substance, including biological, physical, or chemical substances, obtained from a specimen such including a site, sample, person, or population. Each measure is stored in the `Measures table` template as a separate row, following the "long" table format, which is the primary method for storing measures. For example, if you recorded PCR results for two SARS-CoV-2 regions, covN1 and covN2, you would record each measurement on a separate line. Alternatively, you can use a "wide" table format to store multiple measures in a single row or create tables that combine both long and wide headers. The [European Union supersites template](https://osf.io/ab9se/files/osfstorage) is a hybrid template that exemplifies this (found in the "supersites - FKA airports" folder).

(Video)

### 4. Next steps

In addition to recording measures from samples, you can also record measures for a specific site, such as wastewater flow rate ([flowRate](parts.qmd#flowrate)), or the population served by the site, such as the Covid-19 hospitalization rate for a wastewater treatment plant's wasteshed or municipality. **Figure 2** illustrates the these three types of measurements: `sample`, `site`, and `polygon`.

The "who, where, and why" of these measurements are stored in the contact information tables, geospatial tables, and data source & processing tables; while the "how," which refers to the measurement protocol or method, is stored in the protocol or methods tables, and the results tables.

The "who, where, and what else" are extended in version 3 to also integrate public health actions and external dataset linkages. These are included as optional extensions for programs that want to incorporate these items, and operate at this level of detail.

![**Figure 2 - ODM tables and 'subway' lines for storing measures**](assets/intro/Figure2-ODMSubway-complete.png){fig-align="center"}
