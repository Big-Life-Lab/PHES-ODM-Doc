...
-   [`parts`](https://docs.phes-odm.org/parts.html) -- Parts are the smallest way of describing anything in the ODM. An analogy to parts of a car can be used to illustrate this point. If you own a car, your garage can access a parts list that contains every part of your car, right down to every nut and bolt. There are over 2200 parts, all listed in alphabetical order. Each part has up to 60 different attributes such as the part name and description.

-   [`tables`](https://docs.phes-odm.org/tables.html) - The ODM database model has 26 tables in its full relational database. The full model is commonly referred to as "long" tables as it stores data with one measurement per row. The most important tables are the 'report' tables that are used for daily reporting of measures and samples.
