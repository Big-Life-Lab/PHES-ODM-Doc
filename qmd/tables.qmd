...
for (table_row_index in 1:nrow(tables_data)) {
  ...
  table_ID <- single_table_information[[pkg.env$part_ID_column_name]]
  if (is.null(table_ID) ||
  ...
  table_of_content <-
    glue::glue(
      '{table_of_content}<li><a href=\"#{single_table_information[[pkg.env$part_ID_column_name]]}\">{single_table_information[[pkg.env$part_label_column_name]]}</a></li>'
    )
  ...
  table_label <- single_table_information[[pkg.env$part_label_column_name]]
  table_label <-
    modify_and_check_glue_input(table_label, table_ID, pkg.env$part_label_column_name)
  
  table_description <- single_table_information[[pkg.env$part_description_column_name]]
  table_description <- modify_and_check_glue_input(table_description, table_ID, pkg.env$part_description_column_name)
  
  table_instruction <- single_table_information[[pkg.env$part_instruction_column_name]]
  table_instruction <- modify_and_check_glue_input(table_instruction, table_ID, pkg.env$part_instruction_column_name)
  ...
  for (column_row_index in 1:nrow(table_columns)) {
    ...
    if (length(tmp_key_type) > 0 && !is.na(tmp_key_type)) {
      if (tolower(tmp_key_type) == pkg.env$parts_sheet_table_column_type_is_FK) {
        tmp_key_type <- "Foreign Key"
      } else if (tolower(tmp_key_type) == pkg.env$parts_sheet_table_column_type_is_PK) {
        tmp_key_type <- "Primary Key"
      } else if (tolower(tmp_key_type) == pkg.env$parts_sheet_table_column_type_is_header) {
        tmp_key_type <- "Header"
      } else {
        next()
      }
    ...
    column_ID <- column_row[[pkg.env$part_ID_column_name]]
    ...
    column_label <- column_row[[pkg.env$part_label_column_name]]
    column_label <-
      modify_and_check_glue_input(column_label, column_ID, pkg.env$part_label_column_name)
    
    column_description <- column_row[[pkg.env$part_description_column_name]]
    column_description <-
      modify_and_check_glue_input(column_description, column_ID, pkg.env$part_description_column_name)
    ...
    column_dataType <- column_row[[pkg.env$part_data_type_column_name]]
    column_dataType <-
      modify_and_check_glue_input(column_dataType, column_ID, pkg.env$part_data_type_column_name)
    ...
    if (column_dataType == pkg.env$part_sheet_data_type_is_categorical) {
      column_catSetID <- column_row[[pkg.env$part_cat_set_ID_column_name]]
      column_catSetID <-
        modify_and_check_glue_input(column_catSetID, table_ID, pkg.env$part_cat_set_ID_column_name)
