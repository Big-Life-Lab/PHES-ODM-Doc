...
parts_display_template <- '### {partLabel} {{ #{partID} }} 
      ({partID}). {partDesc} {if (partInstr %!=na% constants$dictionary_missing_value_replacement) partInstr else "" } </br>
      ...
      *Missingness Set:* <a href="#{missingnessSet}">{missingnessSet} </br>
      *Minimum Value:* {minValue} </br>
      *Maximum Value:* {maxValue} </br>
      *Minimum Length:* {minLength} </br>
      *Maximum Length:* {maxLength} </br>
      *Part Status:* {if (status %!=na% constants$part_sheet_status_is_active) glue::glue(\'**{status}**\') else status }. \\
