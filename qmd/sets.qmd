...
source("R/sets-module.R")

# Display string templates
header_template <-
  '## {partLabel} {{ #{partID} }}
  <a href=\"/parts.html#{partID}\">({partID})</a>. {partDesc} {if (partInstr %!=na% dictionary$dictionary_missing_value_replacement) partInstr else "" }'

set_sub_header_template <- '<a href=\"/parts.html#{partID}\">({partID})</a>{partDesc} \\
{if(partInstr %!=na% dictionary$dictionary_missing_value_replacement) partInstr else ""} </br>
...
sets_table_source <-
  readxl::read_excel(
    file.path(
      getwd(),
      dictionary$dictionary_directory,
      dictionary_full_file_name
    ),
    sheet = dictionary$sets_sheet_name
  )
...
for (set_type in unique_set_types) {
  ...
  if (set_type == sets_sheet_values$set_type_is_dictSet || set_type == sets_sheet_values$set_type_is_catSet) {
    next()
  }
  ...
  for (set_ID in unique_set_IDs) {
    ...
    for (part_ID in unique_part_IDs) {
      ...
      list_values[[length(list_values) + 1]] <- generate_display(part_bullet_template, part_input, part_ID)
...
# Create lists
list_set_rows <-
  sets_table[sets_table[[sets_sheet_column_names$part_set_ID_column_name]] == sets_sheet_values$set_ID_is_list_set,]
...
categories_info_table <- sets_table[sets_table[[sets_sheet_column_names$part_set_type_column_name]] == sets_sheet_values$set_type_is_catSet, ]
