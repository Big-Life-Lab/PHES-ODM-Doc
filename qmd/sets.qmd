...
sets_table <-
  sets_table[order(sets_table[[sets_sheet_column_names$set_type]]),]
...
unique_set_types <-
  unique(sets_table[[sets_sheet_column_names$set_type]])
for (set_type in unique_set_types) {
  ...
  set_type_info <-
    parts_table[parts_table[[parts_sheet_column_names$part_id]] == set_type,]
  ...
  set_header_input[["partLabel"]] <-
    set_type_info[[parts_sheet_column_names$part_label]]
  set_header_input[["partInstr"]] <-
    set_type_info[[parts_sheet_column_names$part_instruction]]
  set_header_input[["partDesc"]] <-
    set_type_info[[parts_sheet_column_names$part_description]]
  ...
  rows_for_current_set_type <-
    sets_table[sets_table[[sets_sheet_column_names$set_type]] == set_type, ]
  unique_set_IDs <-
    unique(rows_for_current_set_type[[sets_sheet_column_names$set_id]])
  unique_set_IDs <- sort(unique_set_IDs)
  for (set_ID in unique_set_IDs) {
    set_ID_info <-
      parts_table[parts_table[[parts_sheet_column_names$part_id]] == set_ID,]
    ...
    set_ID_part_label <-
      set_ID_info[[parts_sheet_column_names$part_label]]
    ...
    set_ID_input[["partDesc"]] <- set_ID_info[[parts_sheet_column_names$part_description]]
    set_ID_input[["partInstr"]] <- set_ID_info[[parts_sheet_column_names$part_instruction]]
    set_ID_input[["status"]] <- set_ID_info[[parts_sheet_column_names$status]]
    set_ID_input[["firstReleased"]] <- set_ID_info[[parts_sheet_column_names$first_release]]
    set_ID_input[["lastUpdated"]] <- set_ID_info[[parts_sheet_column_names$last_updated]]
    ...
    current_set_ID_rows <-
      rows_for_current_set_type[rows_for_current_set_type[[sets_sheet_column_names$set_id]] == set_ID, ]
    unique_part_IDs <-
      unique(current_set_ID_rows[[parts_sheet_column_names$part_id]])
    unique_part_IDs <- sort(unique_part_IDs)
    for (part_ID in unique_part_IDs) {
      part_info <-
        parts_table[parts_table[[parts_sheet_column_names$part_id]] == part_ID, ]
...
# Create lists
list_set_rows <-
  sets_table[sets_table[[sets_sheet_column_names$set_id]] == sets$set_id$categories$list_set,]
unique_list_part_IDs <-
  unique(list_set_rows[[parts_sheet_column_names$part_id]])
...
for (list_ID in unique_list_part_IDs) {
  list_info_row <-
    parts_table[parts_table[[parts_sheet_column_names$part_id]] ==
                  list_ID,]
  # Verify that parts contains the requested part information
  ...
  list_input[["partLabel"]] <-
    list_info_row[[parts_sheet_column_names$part_label]]
  list_input[["partInstr"]] <-
    list_info_row[[parts_sheet_column_names$part_instruction]]
  list_input[["partDesc"]] <-
    list_info_row[[parts_sheet_column_names$part_description]]
  
  list_display_info <- generate_display(header_template, list_input, list_ID)
   
  
  list_elements_info_rows <-
    parts_table[parts_table[[parts_sheet_column_names$part_type]] == list_ID,]
  list_elements <-
    unique(list_elements_info_rows[[parts_sheet_column_names$part_id]])
  list_elements_list <- list()
  for (element_ID in list_elements) {
    element_info <-
      list_elements_info_rows[list_elements_info_rows[[parts_sheet_column_names$part_id]] == element_ID,]
    # Verify that parts contains the requested part information
...
categories_info_table <- sets_table[sets_table[[sets_sheet_column_names$set_type]] == sets$set_type$categories$mma_set, ]
unique_category_set_IDs <- unique(categories_info_table[[sets_sheet_column_names$set_id]])
