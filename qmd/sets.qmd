...
Some sets can have parts that are reused. For example, there are different quality measures for sample collection (e.g. `Sample not collected`), PCR measures (`inhibition preset`), and sequencing measures (`low depth of coverage`), but all quality sets share the `No quality concerns` indicator.
...
for (set_type in unique_set_types) {
  ...
  display_content <- glue::glue('{display_content}
                              ...
                              partID: <a href=\"/parts.html#{set_type}\">{set_type}</a>. {set_type_description} {if (set_type_instruction %!=na% constants$dictionary_missing_value_replacement) set_type_instruction else "" }
